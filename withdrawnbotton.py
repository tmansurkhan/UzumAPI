...
main_keyboard = ReplyKeyboardMarkup(
    keyboard=[["Hisobot olish"]],
    resize_keyboard=True
)

# --- Inline Keyboard (hisobot vaqt variantlari) ---
...
# --- Hisobot ma'lumotlarini olib kelish ---
def fetch_report(start_date, end_date):
    ...
    top_skus = (
        filtered.groupby('D')['total_sold_price']
        .sum()
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "today":
        ...
        await query.message.reply_text(
            "Ixtiyoriy vaqt oralig'ini kiriting.\nFormat: YYYY-MM-DD - YYYY-MM-DD",
        )
        return  # Userdan matn kutamiz
    else:
...
# --- Ixtiyoriy vaqt matnini qabul qilish ---
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if " - " in text:
        try:
            ...
            if report_data:
                report_text = format_report(start_date, end_date, report_data, top_skus)
            ...
            logging.error(e)
            await update.message.reply_text("Vaqt formatini to'g'ri kiriting. Masalan: 2025-04-01 - 2025-04-27")
    else:
        await update.message.reply_text("Hisobot olish uchun 'Hisobot olish' tugmasidan foydalaning.")
